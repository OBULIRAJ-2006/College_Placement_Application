...
router.put("/basic-info", auth, async (req, res) => {
  try {
    ...
    if (
      ...
      console.log(
        "CGPA update blocked for completed profile, role:",
        user.role
      );
    }
    ...
    try {
      ...
      if (req.files) {
        ...
        if (req.files.marksheets) {
          user.profile.marksheets = req.files.marksheets.map(
            (file) => file.filename
          );
        }
      }
...
router.get("/download/:filename", auth, async (req, res) => {
  try {
    ...
    // Get user to verify they own this file
    const user = await User.findById(req.user.id);
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }

    // Check if filename exists in user's profile
    const fileExists = 
      user.profile?.photo?.includes(filename) ||
      user.profile?.resume?.includes(filename) ||
      user.profile?.collegeIdCard?.includes(filename) ||
      (user.profile?.marksheets && user.profile.marksheets.some(m => m.includes(filename)));
